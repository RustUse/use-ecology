...
FOCUSED_CRATES := use-ecosystem use-habitat use-population use-community use-niche use-biome use-food-web use-species-interaction use-trophic-level use-biodiversity use-conservation-status
...
help:
	@printf "%s\n" \
		...
		"deny                           Run cargo-deny" \
		"sbom                           Generate a CycloneDX SBOM for use-ecology" \
		"publish-dry-run-focused        List package contents and dry-run publish focused crates" \
		"publish-dry-run-facade         Dry-run publish use-ecology after crates.io propagation" \
...
sbom:
	cargo cyclonedx --manifest-path crates/use-ecology/Cargo.toml --all-features --format json --spec-version 1.5 --override-filename sbom.cyclonedx
...
publish-dry-run-facade:
	cargo publish --dry-run --allow-dirty -p use-ecology
